...
public class RunicPlayerData {
    ...
    public Map<ResourceLocation, RunicDiscoveredFormula> getDiscoveredFormulaMap() {
        return discoveredFormulas;
    }

    public Collection<RunicDiscoveredFormula> getDiscoveredFormulas() {
    ...
    public RunicDiscoveredFormula recordFormulaCast(ResourceLocation formulaId, List<ResourceLocation> inputRunes) {
        ...
        discoveredSequences.add(formulaId);

        RunicDiscoveredFormula existing = discoveredFormulas.get(formulaId);
        RunicDiscoveredFormula updated = existing == null
                ? new RunicDiscoveredFormula(formulaId, inputRunes, 1)
                : existing.withAdditionalCast(inputRunes);

        discoveredFormulas.put(formulaId, updated);
        return updated;
    }
    ...
    private ListTag writeDiscoveredFormulas() {
        ListTag list = new ListTag();

        discoveredFormulas.values().stream()
                .sorted(Comparator.comparing(formula -> formula.formulaId().toString()))
                .forEach(formula -> list.add(formula.serializeNBT()));

        return list;
    }

    private void readDiscoveredFormulas(CompoundTag tag) {
        ListTag list = tag.getList(KEY_DISCOVERED_FORMULAS, net.minecraft.nbt.Tag.TAG_COMPOUND);

        for (int i = 0; i < list.size(); i++) {
            RunicDiscoveredFormula formula = RunicDiscoveredFormula.deserializeNBT(list.getCompound(i));
